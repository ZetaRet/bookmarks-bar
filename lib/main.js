...
function createStarModal() {
	var item = document.createElement('div'),
		...
		sel = '<select class="file-select" style="width:100%; color:#000; backgorund-color:#fff; border:none; padding:2px; display:block;">';
	if (editor) {
		p = editor.getPath();
	}
	if (f) {
		...
		f.forEach(function(el, index) {
			sel += '<option value="' + index + '">' + el.n.split('<').join('').split('>').join('') + '</option>';
		});
	}
	sel += '</select>';
	var newfolder = 'Create new folder: ' +
		'<input class="folder-input zetaretbookmarksbarinput" type="text" style="width:100%; color:#000; backgorund-color:#fff; border:none; padding:2px; display:block;"/>';
...
function createFolderModal(fbtn) {
	...
	if (f) {
		...
		f.forEach(function(el, index) {
			var op = el.p.split('<').join('').split('>').join('');
			if (rp === 'yes') op = op.split('\\').reverse().join('\\');
			sel += '<option value="' + index + '">' + op + '</option>';
		});
	}
	sel += '</select>';
	desc.innerHTML = 'Current folder: <b>' + fbtn._name.split('<').join('').split('>').join('') + '</b><br/><br/>' + (f && f.length > 0 ?
		'<b>You are about to view this editor:</b><br/>' + sel :
...
function createFolderBtn(name, files, index) {
	...
	fbtn.innerHTML = name.split('<').join('').split('>').join('');
	fbtn._index = index;
	fbtn._files = files;
	fbtn._name = name;
