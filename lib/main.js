...
var cfg = {
	...
	nskey = cfg.packageid + '.',
	cfgkeys = [
		'bookmarks', 'textColor', 'textGamma', 'fontSize', 'priority', '__jsondata', '__jsonpath',
		'reverseFolderPaths', 'showIcons', 'textHoverColor', 'filesort'
	],
	disposables = null,
	cfgobserve = {},
	fs = null,
	css = [
		...
		'.folder-modal-name:hover { text-decoration: underline; }',
...
var fbtnover, dragging, dragevent, dragclone;

var createTopEl = ui.createTopEl,
	createSpacer = ui.createSpacer,
	createModalBtn = ui.createModalBtn,
	computeColor = utils.computeColor,
	getRGBA = utils.getRGBA;
...
function createFolderModal(fbtn) {
	...
	if (f) {
		var filebuffer = [],
			rp = cfg.reverseFolderPaths || 'no';

		f.forEach(function(el, index) {
			var op = el.p.split(new RegExp('<|>')).join('');
			if (rp === 'yes') op = op.split(path.sep).reverse().join(path.sep);
			filebuffer.push([index, op]);
		});
		if (cfg.filesort !== 'none') {
			filebuffer.sort((a, b) => {
				if (a[1] < b[1]) return (cfg.filesort === 'asc' ? -1 : 1);
				else if (a[1] > b[1]) return (cfg.filesort === 'asc' ? 1 : -1);
				return 0;
			});
		}
		filebuffer.forEach(file => sel += ('<option value="' + file[0] + '">' + file[1] + '</option>'));
	}
	sel += '</select>';
...
function createFolderBtn(name, files, index, fd) {
	...
	fbtntitle = [
		...
		'Drag Folder to move index in bar'
	];
	...
	fbtn.addEventListener('mouseout', function(e) {
		if (fbtnover === fbtn) fbtnover = null;
	...
	fbtn.addEventListener("mousedown", onDragStart);
	return fbtn;
}
...
function createFileBtn(name, pathx, index) {
	...
	fbtntitle = [
		...
		'Drag File to move index in bar'
	];
	fbtn.setAttribute('title', fbtntitle.join('\n'));
	var p = pathx,
		ext, n, icon;
...
function onDragStart(ev) {
	dragging = ev.target;
	dragevent = ev;
	dragclone = dragging.cloneNode(true);
	dragclone.style.position = "absolute";
	dragclone.style.opacity = 0;
	dragclone.style.pointerEvents = "none";
	dragging.style.opacity = 0.6;
	topPanel.element.parentNode.appendChild(dragclone);
}

function onDragMove(ev) {
	if (dragging) {
		dragclone.style.left = (ev.pageX - dragevent.offsetX) + "px";
		dragclone.style.top = (ev.pageY - dragevent.offsetY) + "px";
		if (Math.abs(ev.screenX - dragevent.screenX) > 5 || Math.abs(ev.screenY - dragevent.screenY) > 5) {
			dragclone.style.opacity = 0.4;
		}
	}
}
...
function onDragEnd(ev) {
	if (dragging) {
		if (fbtnover && fbtnover !== dragging) {
			var d;
			if (dragging._path && fbtnover._path) {
				d = jsondata.files.splice(dragging._index, 1)[0];
				jsondata.files.splice(fbtnover._index, 0, d);
				updateJsonData();
			} else if (dragging._files && fbtnover._files) {
				d = jsondata.folders.splice(dragging._index, 1)[0];
				jsondata.folders.splice(fbtnover._index, 0, d);
				updateJsonData();
			}
		}
		dragclone.remove();
		dragging.style.opacity = null;
		dragging = null;
		dragclone = null;
		dragevent = null;
	}
