...
function createFolderBtn(name, files, index) {
	...
	fbtn._files = files;
	fbtn._name = name;
	fbtn.setAttribute('title', 'Click to access folder modal of ' + name + '\nCTRL+Click to open all files in editor\nALT+Click to open all files in explorer');
...
function createFileBtn(name, pathx, index) {
	...
	fbtn.setAttribute('title', 'Click to view file modal of ' + pathx + '\nCTRL+Click to open in editor\nALT+Click to open in explorer');
	var p = pathx,
		ext, n, icon;
	if (p) {
		p = p.split(path.sep);
