...
var cfg = {
	...
	css = [
		...
		'.zetaretbookmarksbarselect { width: 100%; color: #000; background-color: #fff; border: none; padding: 2px; display: block; }',
		'.zetaretbookmarksbarinput { width: 100%; color: #000; background-color: #fff; border: none; padding: 2px; display: block; }',
		'.folder-modal-select { width: 100%; color: #000; background-color: #fff; border: none; padding: 2px; margin-top: 10px; }',
		'.folder-modal-name { display: inline-block; margin: 2px; }',
		'.bookmark-save-folder { margin-left: 4px; border: none; background-color: #000; padding: 2px 13px; color: #fff; border-radius: 5px; cursor: pointer; }',
		'.bookmark-save-folder:hover { background-color: #111; color: #eee; }'
	];
...
function createFolderModal(fbtn) {
	...
	sel += '</select>';
	desc.innerHTML = 'Current folder: <b><span class="folder-name folder-modal-name">' +
	...
	if (fnel) {
		...
		fnel.addEventListener('click', function(e) {
			...
			function savefinp() {
				jsondata.folders[fbtn._index].n = iv;
				updateJsonData();
				item._panel.destroy();
			}
